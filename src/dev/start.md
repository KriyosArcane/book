...
By default, [w64-exec-calc-shellcode-func.bin](https://github.com/peterferrie/win-exec-calc-shellcode/blob/master/build/bin/w64-exec-calc-shellcode-func.bin) will be run,
and you should see the `calc` program start.
...
(we call it `Prefix`, PumpBin uses it to locate the placeholder data, so `Prefix` needs to be unique, PumpBin will use the first match).
...
The complete project files for this example are available in the PumpBin code repository at [examples/create_thread](https://github.com/pumpbin/pumpbin/blob/main/examples/create_thread/src/main.rs).
